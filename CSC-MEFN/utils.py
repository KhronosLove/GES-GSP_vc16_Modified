...
def test(net, loader, save_path):
    ...
    # Transformation to resize the image to 1080p
    resize_transform = transforms.Resize(resolution)

    # 1. create the output path
    mkdir(os.path.join(save_path))
    ...
    with torch.no_grad():
        ...
        for i, img in enumerate(loader):
            ...
            imsave(os.path.join(save_path,file_name), imgf995)
            #imsave(os.path.join(save_path,'test_%s'%(identifier),file_name),       imgf)
            print('%s: %dth image saved...')